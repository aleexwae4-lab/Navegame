...
const ui = {
  ...
  heatBar: $('#heat-bar'), comboLabel: $('#combo-label'), bossCard: $('#boss-card'), bossBar: $('#boss-bar'), bossHp: $('#boss-hp-text'), bossPhase: $('#boss-phase'),
  secondaryBar: $('#secondary-bar'), secondaryLabel: $('#secondary-label'), secondaryButton: $('#secondary-btn'), biomeLabel: $('#biome-label'), activePerks: $('#active-perks'),
  startScreen: $('#start-screen'), pauseScreen: $('#pause-screen'), perkScreen: $('#perk-screen'), perkGrid: $('#perk-grid'), gameOverScreen: $('#game-over-screen'), fatalScreen: $('#fatal-screen'), fatalMessage: $('#fatal-message'),
...
function announce(message) {
  ...
  announceTimer = window.setTimeout(() => ui.announcer.classList.remove('show'), 1350);
}
...
function handleState(state, snapshot) {
  if (state === 'playing' || state === 'paused' || state === 'perk') closeMetaScreens();
  ...
  ui.pauseButton.disabled = state === 'idle' || state === 'gameover' || state === 'perk';
...
document.addEventListener('click', (event) => {
  ...
  if (perkButton) {
    game.choosePerk(perkButton.dataset.perkId);
    return;
  }
