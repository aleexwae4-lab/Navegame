...
import { CONTRACTS, FACTIONS, PERKS } from './config.js';
...
function installV10Ui() {
  ...
  coach.id = 'v10-coach';
  coach.className = 'v10-coach hidden';
  coach.setAttribute('role', 'status');
  app?.append(coach);

  const impact = document.createElement('div');
  impact.id = 'v10-impact';
  impact.className = 'v10-impact';
  impact.setAttribute('aria-hidden', 'true');
  app?.append(impact);

  const makeMastery = (id) => {
    const el = document.createElement('div');
    el.id = id;
    el.className = 'v10-mastery-card';
    el.innerHTML = '<span>MAESTRÍA</span><strong>CADETE · 0 XP</strong><small>Juega con precisión para subir de rango.</small>';
    return el;
  };

  const startPanel = $('#start-screen .panel');
  const startButton = $('#start-btn');
  const startMastery = makeMastery('v10-start-mastery');
  if (startPanel && startButton) startPanel.insertBefore(startMastery, startButton);

  const gameOverPanel = $('#game-over-screen .panel');
  const restartButton = $('#restart-btn');
  const gameOverMastery = makeMastery('v10-gameover-mastery');
  if (gameOverPanel && restartButton) gameOverPanel.insertBefore(gameOverMastery, restartButton);
}
...
const ui = {
  ...
  objectiveCard: $('#side-objective-card'), objectiveTitle: $('#side-objective-title'), objectiveReward: $('#side-objective-reward'), objectiveBar: $('#side-objective-bar'), objectiveProgress: $('#side-objective-progress-text'),
  ...
  factionLabel: $('#faction-label'), doctrineLabel: $('#doctrine-label'), campaignRank: $('#campaign-rank'), campaignChapter: $('#campaign-chapter'), campaignRenown: $('#campaign-renown'),
  ...
  finalCampaignRank: $('#final-campaign-rank'), finalRenown: $('#final-renown'), finalObjectives: $('#final-objectives'),
  loadoutLabel: $('#loadout-label'), profileCores: $('#profile-cores'), selectedSectorLabel: $('#selected-sector-label'), startButton: $('#start-btn'),
  hangarCores: $('#hangar-cores'), shipGrid: $('#ship-grid'), weaponGrid: $('#weapon-grid'), galaxyGrid: $('#galaxy-grid'), galaxyBest: $('#galaxy-best-sector'), factionGrid: $('#faction-grid'),
  eliteKills: $('#elite-kills'), guardiansDefeated: $('#guardians-defeated'), minibossesDefeated: $('#minibosses-defeated'),
  coach: $('#v10-coach'), impact: $('#v10-impact'), startMastery: $('#v10-start-mastery'), gameOverMastery: $('#v10-gameover-mastery'),
};
...
function coach(message, duration = 2300) {
  if (!ui.coach || !message) return;
  window.clearTimeout(coachTimer);
  ui.coach.textContent = message;
  setVisible(ui.coach, true);
  requestAnimationFrame(() => ui.coach.classList.add('show'));
  coachTimer = window.setTimeout(() => {
    ui.coach.classList.remove('show');
    window.setTimeout(() => setVisible(ui.coach, false), 180);
  }, duration);
}

function impactFeedback({ grace = false } = {}) {
  window.clearTimeout(impactTimer);
  document.body.classList.add('v10-impact-active');
  if (ui.impact) ui.impact.dataset.grace = grace ? 'true' : 'false';
  impactTimer = window.setTimeout(() => document.body.classList.remove('v10-impact-active'), 150);
}
...
function renderObjective(objective) {
  if (!objective || !ui.objectiveCard) return;
  const percent = Math.min(100, (objective.progress / Math.max(1, objective.target)) * 100);
  ui.objectiveTitle.textContent = objective.complete ? 'OBJETIVO COMPLETADO' : objective.name;
  ui.objectiveReward.textContent = `+${objective.rewardCores}C · +${objective.rewardRenown}R`;
  ui.objectiveBar.style.width = `${percent}%`;
  const progress = objective.id === 'shield' ? `${objective.progress.toFixed(1)}s / ${objective.target}s` : `${Math.floor(objective.progress)} / ${objective.target}`;
  ui.objectiveProgress.textContent = objective.complete ? 'RECOMPENSA COBRADA' : `${objective.description} · ${progress}`;
  ui.objectiveCard.classList.toggle('complete', objective.complete);
}

function renderCampaign(campaign) {
  if (!campaign) return;
  if (ui.factionLabel) ui.factionLabel.textContent = campaign.faction?.name ?? 'GALAXIA';
  if (ui.doctrineLabel) ui.doctrineLabel.textContent = campaign.faction?.doctrine ?? 'DOCTRINA ABIERTA';
  if (ui.campaignRank) ui.campaignRank.textContent = campaign.rank ?? 'PILOTO';
  if (ui.campaignChapter) ui.campaignChapter.textContent = `CAP. ${campaign.chapter ?? 1}`;
  if (ui.campaignRenown) ui.campaignRenown.textContent = `${campaign.renown ?? 0} R`;
  renderObjective(campaign.objective);
}

function renderMastery(meta = {}) {
  const rank = meta.masteryRank ?? 'CADETE';
  const xp = meta.masteryXp ?? 0;
  const clean = meta.cleanSectors ?? 0;
  for (const element of [ui.startMastery, ui.gameOverMastery]) {
    if (!element) continue;
    element.querySelector('strong').textContent = `${rank} · ${xp} XP`;
    element.querySelector('small').textContent = clean > 0 ? `${clean} sector${clean === 1 ? '' : 'es'} perfecto${clean === 1 ? '' : 's'} · precisión recompensada` : 'Combos, objetivos y Guardianes aumentan tu Maestría.';
  }
}
...
function renderHud(state) {
  ...
  if (state.boss) {
    const percent = Math.max(0, (state.boss.hp / state.boss.maxHp) * 100);
    ui.bossBar.style.width = `${percent}%`;
    ui.bossHp.textContent = `${Math.ceil(percent)}%`;
...
function renderFactionOverview(meta) {
  if (!ui.factionGrid) return;
  const wins = meta.factionVictories ?? {};
  ui.factionGrid.innerHTML = Object.values(FACTIONS).map((faction) => `<article class="faction-card"><span>${faction.doctrine}</span><strong>${faction.name}</strong><b>${wins[faction.id] ?? 0}</b></article>`).join('');
}
...
function renderMeta(meta) {
  ...
  if (ui.finalCampaignRank) ui.finalCampaignRank.textContent = meta.campaignRank ?? 'PILOTO';
  if (ui.finalRenown) ui.finalRenown.textContent = (meta.campaignRenown ?? 0).toLocaleString('es-MX');
  if (ui.finalObjectives) ui.finalObjectives.textContent = (meta.sideObjectivesCompleted ?? 0).toLocaleString('es-MX');
  if (ui.campaignRank) ui.campaignRank.textContent = meta.campaignRank ?? ui.campaignRank.textContent;
  if (ui.campaignChapter) ui.campaignChapter.textContent = `CAP. ${meta.campaignChapter ?? 1}`;
  if (ui.campaignRenown) ui.campaignRenown.textContent = `${meta.campaignRenown ?? 0} R`;
  if (ui.startButton && (game?.state === 'idle' || game?.state === 'gameover')) ui.startButton.textContent = `JUGAR · SECTOR ${meta.selectedSector}`;
  renderMastery(meta);
  renderCatalog(meta);
  renderGalaxy(meta);
  renderFactionOverview(meta);
}
...
function clearSteering() {
  for (const direction of ['up', 'down', 'left', 'right']) game.setInput(direction, false);
}
...
function handleState(state, snapshot) {
  ...
  if (state !== 'playing') clearSteering();
  if (state === 'gameover') {
...
const game = new NeonRiderGame($('#game-container'), {
  ...
  onFaction: (faction) => {
    if (!faction) return;
    ui.factionLabel.textContent = faction.name;
    ui.doctrineLabel.textContent = faction.doctrine;
  },
  onObjective: renderObjective,
  onCampaign: ({ rank, chapter, renown }) => {
    ui.campaignRank.textContent = rank;
    ui.campaignChapter.textContent = `CAP. ${chapter}`;
    ui.campaignRenown.textContent = `${renown} R`;
    cinematic(`CAMPAÑA · ${rank}`);
  },
  ...
  onCoach: ({ text }) => coach(text),
  onImpact: impactFeedback,
  onMastery: ({ amount, label, mastery, rankUp }) => {
    renderMastery({ masteryRank: mastery.rank, masteryXp: mastery.xp, cleanSectors: mastery.cleanSectors });
    if (rankUp) cinematic(`MAESTRÍA · ${mastery.rank}`);
    else coach(`+${amount} XP · ${label}`, 1500);
  },
  onState: handleState,
  onAnnounce: announce,
...
// V10 mobile steering: one finger can drag on open space while another holds FIRE.
...
function updateDragSteering(x, y) {
  const dx = x - dragX;
  const dy = y - dragY;
  game.setInput('left', dx < -dragThreshold);
  game.setInput('right', dx > dragThreshold);
  game.setInput('up', dy < -dragThreshold);
  game.setInput('down', dy > dragThreshold);
}

playfield?.addEventListener('pointerdown', (event) => {
  if (event.pointerType !== 'touch' || game.state !== 'playing' || dragPointer !== null) return;
...
playfield?.addEventListener('pointermove', (event) => {
  if (event.pointerId !== dragPointer || game.state !== 'playing') return;
  updateDragSteering(event.clientX, event.clientY);
  event.preventDefault();
});
const endDrag = (event) => {
...
playfield?.addEventListener('lostpointercapture', (event) => { if (event.pointerId === dragPointer) { dragPointer = null; clearSteering(); } });

document.addEventListener('visibilitychange', () => { if (document.hidden && game.state === 'playing') game.pause(); });
window.addEventListener('blur', () => { game.clearInput(); dragPointer = null; });
renderMeta(game.metaSnapshot());
