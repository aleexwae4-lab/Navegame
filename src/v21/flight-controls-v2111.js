...
const STEERING_ACTION_GUTTER = 154;
...
function playerSettings() {
  try {
    const live = window.__waeNeonSettings?.get?.();
    if (live) return live;
...
function controlScheme() {
  return playerSettings()?.controls?.scheme === 'joystick' ? 'joystick' : 'classic';
}
...
function hapticsEnabled() {
  return playerSettings()?.controls?.haptics !== false;
}

function leftHanded() {
  return playerSettings()?.controls?.leftHanded === true;
}

function sensitivityScale() {
  const value = Number(playerSettings()?.controls?.sensitivity);
  return clamp(Number.isFinite(value) ? value / 100 : 0.9, 0.7, 1.15);
}

function joystickSizeScale() {
  const value = Number(playerSettings()?.controls?.joystickSize);
  return clamp(Number.isFinite(value) ? value / 100 : 1, 0.8, 1.25);
}

function tuning() {
  const preset = playerSettings()?.controls?.preset;
  return TUNING[preset] || TUNING.balanced;
}
...
function steeringSpan() {
  const ratioLimit = window.innerWidth * STEERING_ZONE_RATIO;
  const actionClearance = window.innerWidth - STEERING_ACTION_GUTTER;
  return Math.max(148, Math.min(ratioLimit, actionClearance));
}

function pointerInSteeringZone(clientX) {
  const span = steeringSpan();
  return leftHanded() ? clientX >= window.innerWidth - span : clientX <= span;
}

function stickRadius() {
  const base = clamp(Math.min(window.innerWidth, window.innerHeight) * 0.175, 62, 74);
  return base * joystickSizeScale();
}
...
function radialResponse(magnitude) {
  ...
  if (normalized <= profile.pivot) {
    const precision = normalized / profile.pivot;
    power = Math.pow(precision, profile.exponent) * profile.midPower;
  } else {
    const outer = smoothstep((normalized - profile.pivot) / (1 - profile.pivot));
    power = profile.midPower + outer * (1 - profile.midPower);
  }

  return clamp(power * sensitivityScale(), 0, 1);
}
...
function beginSteering(event) {
  const g = game();
  if (!joystickEnabled() || event.pointerType !== 'touch' || g?.state !== 'playing' || flight.pointerId !== null) return;
  if (!pointerInSteeringZone(event.clientX)) return;
...
function moveSteering(event) {
  if (!joystickEnabled() || event.pointerId !== flight.pointerId) return;
...
function ensureFlightSurface() {
  ...
  surface.innerHTML = `
    <div class="v21-flight-stick v2111-stick" aria-hidden="true">
...
function installGameHook() {
  ...
  const wrapped = (delta, elapsed, ...args) => {
    const activeAnalog = touchLayout() && joystickEnabled();
    ...
    if (activeAnalog) {
      ...
      const accel = releasing
        ? profile.releaseAccel
        : reversingX || reversingY
          ? profile.reversalAccel
          : profile.accel;
      ...
      if (flight.pointerId !== null || Math.abs(flight.currentX) + Math.abs(flight.currentY) > 0.005) {
        ...
        const ship = g.ship;
        if (ship?.position) {
          const speed = BASE_SHIP_SPEED * profile.speed * shipSpeedMultiplier(g);
    ...
    } else if (flight.pointerId !== null || flight.currentX || flight.currentY) {
      resetSteering({ hard: true });
    }

    const result = baseUpdate(delta, elapsed, ...args);

    if (activeAnalog && g.ship?.rotation && (flight.pointerId !== null || Math.abs(flight.currentX) + Math.abs(flight.currentY) > 0.005)) {
      const profile = tuning();
      const magnitude = clamp(Math.hypot(flight.currentX, flight.currentY), 0, 1);
      const rollTarget = -flight.currentX * (0.24 + magnitude * 0.08) * profile.attitude;
      const pitchTarget = flight.currentY * (0.11 + magnitude * 0.04) * profile.attitude;
  ...
  Object.defineProperty(wrapped, '__waeV2111Analog', { value: true });
...
function syncPremiumState() {
  ...
  const touch = playing && touchLayout();
  const analog = touch && joystickEnabled();
  const surface = ensureFlightSurface();

  surface.classList.toggle('hidden', !analog);
  document.body.classList.toggle('v21-playing', playing);
  document.body.classList.toggle('v21-touch-flight', touch);
  document.body.classList.toggle('v21-control-joystick', touch && joystickEnabled());
  document.body.classList.toggle('v21-control-classic', touch && !joystickEnabled());

  applyPremiumShipScale(playing);
  installGameHook();
  if (!analog && flight.pointerId !== null) resetSteering({ hard: true });
}

function installActionHaptics() {
  document.addEventListener('pointerdown', (event) => {
    if (!hapticsEnabled() || event.pointerType !== 'touch') return;
    const action = event.target.closest('#fire-btn, #dash-btn, #missile-btn, #secondary-btn, #pause-btn, [data-v21-hud-toggle], [data-settings-open], [data-setting-option]');
    if (!action) return;
...
window.setInterval(syncPremiumState, 240);
...
document.addEventListener('wae:settings-changed', () => {
  resetSteering({ hard: true });
  game()?.clearInput?.();
  syncPremiumState();
});
