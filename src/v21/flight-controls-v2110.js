...
function controlScheme() {
  const value = playerSettings()?.controls?.scheme;
  return value === 'joystick' ? 'joystick' : 'classic';
}
...
function radialResponse(magnitude) {
  ...
  if (normalized <= 0.62) {
    ...
    power = Math.pow(precision, 1.55) * 0.56;
  } else {
    const outer = smoothstep((normalized - 0.62) / 0.38);
    power = 0.56 + outer * 0.44;
  }
...
function ensureFlightSurface() {
  ...
  surface.innerHTML = `
    <div class="v21-flight-stick v2110-stick" aria-hidden="true">
...
function installGameHook() {
  ...
  Object.defineProperty(wrapped, '__waeV2110Analog', { value: true });
...
function installActionHaptics() {
  document.addEventListener('pointerdown', (event) => {
    ...
    const action = event.target.closest('#fire-btn, #dash-btn, #missile-btn, #secondary-btn, #pause-btn, [data-v21-hud-toggle], [data-settings-open]');
    if (!action) return;
...
window.setInterval(syncPremiumState, 260);
